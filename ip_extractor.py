...
class IPExtractor:
    ...
    def consolidate_overlapping_ranges(self, ip_ranges: Set[str]) -> Set[str]:
        ...
        # Convert to network objects and sort by network address
        networks = []
        ...
        for network in networks:
            ...
            # Check if current network overlaps with the last consolidated one
            last_network = consolidated[-1]
            
            # If current network is completely contained within the last one, skip it
            if network.subnet_of(last_network):
                continue
            
            # If current network completely contains the last one, replace it
            if last_network.subnet_of(network):
                consolidated[-1] = network
                continue
            
            # If they overlap but neither contains the other, keep both
            if network.overlaps(last_network):
                consolidated.append(network)
            else:
                # No overlap, add to consolidated list
                consolidated.append(network)
